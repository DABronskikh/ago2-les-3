...
func (s *Service) GetPayments(ctx context.Context, userID int64) ([]*dto.PaymentsDTO, error) {
	paymentsDB := []*dto.PaymentsDTO{}

	sql := `
		SELECT id, senderId, amount
		FROM payments 
		WHERE senderId = $1
		LIMIT 50
	`
	...
	for rows.Next() {
		paymentEl := &dto.PaymentsDTO{}
		err = rows.Scan(&paymentEl.Id, &paymentEl.SenderId, &paymentEl.Amount)
		if err != nil {
			return nil, ErrDB
		}
		paymentsDB = append(paymentsDB, paymentEl)
	}
...
func (s *Service) GetPaymentsAdmin(ctx context.Context) ([]*dto.PaymentsDTO, error) {
	paymentsDB := []*dto.PaymentsDTO{}
...
func (s *Service) CreatePayments(ctx context.Context, id int64, amount int) (dto.PaymentsDTO, error) {
	paymentDB := dto.PaymentsDTO{}

	err := s.pool.QueryRow(ctx, `
		INSERT INTO payments (senderId, amount) VALUES ($1, $2) RETURNING id, senderId, amount
		`, id, amount).Scan(&paymentDB.Id, &paymentDB.SenderId, &paymentDB.Amount)

	if err != nil {
		return paymentDB, ErrDB
	}
